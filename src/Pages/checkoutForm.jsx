...
const CheckoutForm = ({ total, onSuccess }) => {
  ...
  const fetchClientSecret = useCallback(async () => {
    ...
    try {
      ...
      console.error("Stripe API error:", err, "Response:", err.response?.data);
    }
  }, [total]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {error && <p className="text-sm text-center text-gray-900">{error}</p>}
      <button
        type="submit"
    ...
    return (
      <section className="min-h-screen bg-main-100 py-16 poppins">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-2xl bg-main-200 p-8 text-center shadow-lg">
            <FaShoppingCart className="mx-auto h-16 w-16 text-gray mb-4" />
            <p className="text-lg text-gray mb-4">Your cart is empty.</p>
            <Link
              to="/Store"
              className="inline-block rounded-full bg-primary-800 px-6 py-3 text-sm font-medium text-main-100 transition-all hover:bg-main-300 hover:text-zinc"
            >
                  ...
                  <div key={item.id} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-bold text-main-300">
                        ${(item.product.prix * item.quantite).toLocaleString()}
                      </p>
                    </div>
                  </div>
                ))}
          ...
          <div className="order-summary mt-8 w-full max-w-md lg:mt-0">
            <div className="sticky top-6 rounded-2xl bg-main-200 shadow-lg">
              <div className="bg-main-100 p-6">
                <p className="text-xl font-bold text-gray-900">Order Summary</p>
              </div>
              <div className="p-6">
                <div className="space-y-4">
                  <dl className="flex items-center justify-between gap-4">
                    <dt className="text-sm text-gray">Subtotal</dt>
                    <dd className="text-sm font-medium text-gray-900">
                      ${subtotal.toLocaleString()}
                    </dd>
                  </dl>
                  <dl className="flex items-center justify-between gap-4">
                    <dt className="text-sm text-gray">Savings</dt>
                    <dd className="text-sm font-medium text-yellow-500">
                      -${savings.toLocaleString()}
                    </dd>
                  </dl>
                  <dl className="flex items-center justify-between gap-4">
                    <dt className="text-sm text-gray">Store Pickup</dt>
                    <dd className="text-sm font-medium text-gray-900">
                      ${pickup.toLocaleString()}
                    </dd>
                  </dl>
                  <dl className="flex items-center justify-between gap-4">
                    <dt className="text-sm text-gray">Tax</dt>
                    <dd className="text-sm font-medium text-gray-900">
                      ${tax.toLocaleString()}
                    </dd>
                  </dl>
